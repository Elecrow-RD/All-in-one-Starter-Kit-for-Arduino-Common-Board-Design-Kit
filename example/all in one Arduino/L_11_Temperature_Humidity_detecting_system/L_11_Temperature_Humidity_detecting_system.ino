...
void loop() {
  ...
  while (1)
  {
    if (millis() - DHT.lastRead() >= 1000) {  // Check if 1 second has passed since the last reading
      ...
      lcd.print(String(DHT.getTemperature()) + " C");  // Print the temperature value on the first row
      lcd.setCursor(5, 1);  // Set cursor to the 6th column of the second row
      lcd.print(String(DHT.getHumidity()) + " %");  // Print the humidity value on the second row
    }
